    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if dict['Success']:
                ...
                return redirect(url_for('ecolvin'))
            else:
...
@app.route('/ecolvin')
def ecolvin():
    return render_template('ecolvin.html', title='eColvin')

@app.route('/')
@app.route('/index')

def index():
    user = {'username': 'ecolvin'}
...
def jsonTest():
    # return jsonify(list(range(5)))
        instructor = {"username": "ecolvin","role": "student","uid": 11,"name":{"firstname": "Elayne","lastname": "Colvin"}}    
...
def loginAPI():
    ...
    if username == 'ecolvin' and password == '123':
